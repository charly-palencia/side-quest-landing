...
const Card = ({style, customStyle, slide, className, ...props}) => {
  ...
  >
    ...
    <div className="card-details text-white w-full h-48 absolute p-4 text-2xl bottom-0 left-0 z-10">
      <div className="flex"><a href="#" className="tag hover:opacity-100">{slide.license}</a></div>
      <div className="flex">{([...new Array(5)]).map((_,index) =>
                                                     index + 1 <= roundedRating ? <StarIcon width={16}/> : <StartIconOutline width={16}/>
      )}
      </div>
